...
public class TcHashSessionIdManager extends AbstractLifeCycle implements SessionIdManager {
  ...
  /**
   * Get the workname. If set, the workername is dot appended to the session ID and can be used to assist session
  ...
  /**
   * Set the workname. If set, the workername is dot appended to the session ID and can be used to assist session
   * affinity in a load balancer.
   * 
   * @param workerName
   */
  ...
  protected void doStop() {
    if (_sessions != null) _sessions.clear(); // Maybe invalidate?
    _sessions = null;
  }
  ...
  public void invalidateAll(String id) {
    synchronized (this) {
      ...
      while (_sessions.containsKey(id)) {
        ...
        if (isValid) session.invalidate();
        else _sessions.removeValue(id, session);
      }
    }
  }
  ...
  /* ------------------------------------------------------------ */
  /*
   * new Session ID. If the request has a requestedSessionID which is unique, that is used. The session ID is created as
   * a unique random long XORed with connection specific information, base 36.
   * @param request
   * @param created
   * @return Session ID.
   */
  public String newSessionId(HttpServletRequest request, long created) {
    synchronized (this) {
      ...
      while (id == null || id.length() == 0 || idInUse(id)) {
        long r = _weakRandom ? (hashCode() ^ Runtime.getRuntime().freeMemory() ^ _random.nextInt() ^ (((long) request
