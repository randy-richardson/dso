...
public class ClientConnectEventHandler extends AbstractEventHandler {
  ...
  private final NotificationListener tcOperatorEventListener;
  ...
  public ClientConnectEventHandler(final StatisticsGateway statisticsGateway,
                                   NotificationListener tcOperatorEventListener) {
    this.statisticsGateway = statisticsGateway;
    this.tcOperatorEventListener = tcOperatorEventListener;
  }
    ...
    synchronized (channelIdToJmxConnector) {
      if (!channelIdToJmxConnector.containsKey(channel.getChannelID())) {
        ...
        try {
          ...
          ClientBeanBag bag = new ClientBeanBag(l2MBeanServer, this.tcOperatorEventListener, channel, uuid,
  ...
  private static class ClientBeanBag {
    ...
    public ClientBeanBag(MBeanServer l2MBeanServer, NotificationListener tcOperatorEventListener,
                         MessageChannel channel,
                         UUID id, MBeanServerConnection l1Connection) {
          ...
          if (beanNames.add(modifiedObjName)) {
            try {
              ...
              if (modifiedObjName.toString().contains("Terracotta Operator Events Bean")) {
                l2MBeanServer.addNotificationListener(modifiedObjName, this.tcOperatorEventListener, null, channel
                    .getRemoteNodeID());
              }
            } catch (Throwable t) {
    ...
    synchronized void unregisterBean(ObjectName name, boolean remove) {
      ...
      if (beanNames.contains(modifiedObjName)) {
        try {
          // TODO: remove the notification listener
